...
def build_lab2(student_name: str, role: str) -> dict:
    ...
    cells.append(make_markdown_cell([
        ...
        f"**🔧 Code to type:**",
        f"```python",
        f"# Try parsing as YYYY-MM-DD first, then as DD/MM/YYYY",
        f"registry_df = registry_df.withColumn(",
        f"    'Registration_Date',",
        f"    F.coalesce(",
        f"        F.to_date(F.col('Registration_Date'), 'yyyy-MM-dd'),",
        f"        F.to_date(F.col('Registration_Date'), 'dd/MM/yyyy')",
        f"    )",
        f")",
        f"",
        ...
        f"",
        f"**📖 Line-by-line explanation:**",
        f"- `F.to_date(col, format)`: Attempts to parse a string column into a date using the ",
        f"  specified format pattern. Returns NULL if the string doesn't match the pattern.",
